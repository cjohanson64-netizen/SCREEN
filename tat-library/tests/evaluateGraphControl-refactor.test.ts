...
import { evaluateGraphControlExpr } from "../runtime/graphControl/controlEvaluator";
import { evaluateComputeSum } from "../runtime/graphControl/deriveEvaluator";
import { evaluateRepeatCount } from "../runtime/graphControl/repeatEvaluator";
import { evaluateDerivePath } from "../runtime/graphControl/pathTraversal";
import { evaluateGraphQuery } from "../runtime/graphControl/queryEvaluator";
...
describe("evaluateGraphControl refactor behavior", () => {
  ...
  it("evaluates derived sum from collected state values", () => {
    ...
    const result = evaluateComputeSum(graph, sumExpr, {
      bindings: {
        values: new Map(),
        nodes: new Map(),
      },
  ...
  it("evaluates repeat count from a derived numeric expression", () => {
    const graph = createGraphControlTestGraph();
    ...
    const result = evaluateRepeatCount(graph, countExpr, {
      bindings: {
        values: new Map(),
        nodes: new Map(),
      },
