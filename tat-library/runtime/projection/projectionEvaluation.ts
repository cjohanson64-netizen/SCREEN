...
import { cloneGraphValue, getIncomingEdges, getOutgoingEdges } from "../graph/graph.js";
...
function evaluateProjectionDirective(
  ...
  switch (expr.name) {
    ...
    case "@select.from": {
      ...
      // Phase 5 V1: @select.from owns selection; where body is parsed and preserved.
      // Predicate-aware evaluation can be tightened in the Phase 6/semantic runtime pass.
      return value.length > 0 ? cloneGraphValue(value[0]!) : null;
    }
