...
export function parseGraphPipelineSectionImpl(this: any): GraphPipelineStepNode[] {
...
export function parseCtxExprImpl(this: any, startToken?: Token): CtxExprNode {
    const start = startToken ?? this.expect("KEYWORD", "@ctx");
    this.expect("LPAREN");
    const args = this.parseArguments();
    this.expect("RPAREN");

    return makeNode<CtxExprNode>(this, 
      "CtxExpr",
      {
        name: "@ctx",
        args,
...
export function parseTerminalGraphExprImpl(this: any): TerminalGraphExprNode {
    if (this.check("KEYWORD", "@project.apply")) {
      return this.parseProjectExpr();
    }

    throw this.error(
      this.peek(),
      'Expected @project.apply(...) after "<>"',
    );
  }
